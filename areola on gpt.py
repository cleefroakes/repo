...
knowledge_base = {
    ...
    "speaker of the house": "The Speaker of the House is Mike Johnson, a Republican from Louisiana, elected in 2023 and reelected in 2025. Hes the 56th Speaker, second in line to the presidency after the VP.",
    ...
    "trump": "Donald Trump is the 47th President of the United States, serving his second term since January 20, 2025. Hes a businessman, former reality TV star, and leads the Trump Organization.",
...
question_mappings = {
    ...
    r"who('s| is)?\s*(the)?\s*president\s*(of\s*(.*))\??": r"\4",  # Extract country for presidents
...
def get_wikipedia_summary(query, context=None):
    ...
    try:
        ...
        # Try Wikipedia page with variations
        variations = [
            query,
            query.title(),
            query.capitalize(),
...
def chatbot_response(user_input):
    ...
    for pattern, target in question_mappings.items():
        ...
        if match:
            query = match.group(4 if 'president' in pattern or 'capital' in pattern else 2) if match.groups() and len(match.groups()) >= 4 else target
